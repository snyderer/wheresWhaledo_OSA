classdef brushTDOA < handle
    ...
    methods
        function obj = brushTDOA(data, params)
            if nargin==1 % no user-provided params, set to default
                params.source = "default";
            elseif nargin==2 % override default with user-provided params
        ...
        function setParams(obj, params)
            ...
            if isfield(params, "figPosition")
                ...
                monPos = get(0, "MonitorPositions"); % monitor positions
                [~, idxBiggerMon] = max((monPos(:,3).*monPos(:,4))); % determine which monitor is biggest

                obj.params.figPosition = monPos(idxBiggerMon, :);
                set(obj.fig, "Position", obj.params.figPosition)
