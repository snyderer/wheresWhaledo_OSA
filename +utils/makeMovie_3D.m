function errorMessage = makeMovie_3D(whaleTable, saveFileName, speedUpRate, cameraAngle, cameraAngle_timeStamps, params)
...
xLimValues = ([min(x, [], 'all')-(x(2)-x(1)), max(x, [], 'all')]);
yLimValues = ([min(x, [], 'all'), max(x, [], 'all')]);
zLimValues = ([min(x, [], 'all'), max(x, [], 'all')]);
