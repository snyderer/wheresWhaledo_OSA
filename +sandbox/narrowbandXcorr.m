...
for ip = 1:size(PING, 1)
    ...
    x = audioread(fullfile(wav_directory(wav_file_idx).folder, wav_directory(wav_file_idx).name), [idxStart, idxEnd]);
    xf = filtfilt(b, a, x);

    if max(xf, [], 'all')>=th
%         [pks, locs] = findpeaks(xf(:, 1), 'MinPeakHeight', th, 'MinPeakDistance', .25*fs);
        idet = idet+1;
        ...
        for ixc = 1:Ntdoa
            ...
            TDOAex(idet, ixc) = expected_TDOA(ip, ixc);
        end
    end
...
for ilab = 1:length(ulab)
    ...
    for i = 1:length(idx)
        ...
        hold on
        srcIdx = find(ping_numbers==ulab(ilab));
        scatter(xsrc, ysrc, [], colors(ilab, :), 'x')
    end
end
hold off