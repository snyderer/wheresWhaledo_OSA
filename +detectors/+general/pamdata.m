...
%             obj.userParams.filterType.description = 'Filter type';
        ...
        function setInternalParams(obj)
            ...
            obj.internalParams.recTable = obj.wheresWhaledo.arrayPanel.receiverTableMeters.Data; % receiver table
        ...
        function DET = run(obj, wavfile) % run detector
           ...
           fileStartTime = datenum(strrep(wavfile(end-18:end-4),'_','-'), 'yyyymmdd-HHMMSS'); % Fixed issue with '-' (JS)
            ...
            if obj.userParams.rejFilterFreq > 0 && obj.userParams.rejFilterFreq < fs/2
                useBandRejectFilter = true;
                frej = (obj.userParams.rejFilterFreq + [-300, 300]);
                frej(1) = max([frej(1), 1]);
                frej(2) = min([frej(2), fs/2-1]);
                [breject, areject] = butter(obj.userParams.rejFilterOrder, (frej).*2./fs, 'stop');
            else
                useBandRejectFilter = false;
            end
            numDet = height(T);
